...
void check_elf(unsigned char *e_ident)
{
	...
	for (tryindex = 0; tryindex < 4; tryindex++)
	{
		if (e_ident[tryindex] != 127 &&
		    e_ident[tryindex] != 'E' &&
		    e_ident[tryindex] != 'L' &&
		    e_ident[tryindex] != 'F')
		{
...
void print_magic(unsigned char *e_ident)
{
	...
	while (tryindex < EI_NIDENT)
	{
		printf("%02x", e_ident[tryindex]);

		if (tryindex == EI_NIDENT - 1)
			printf("\n");
		else
			printf(" ");
 ...
 * @e_entry: The address.
 * @e_ident: A pointer to an arraythat contains the ELF class.
 */
